...
export function ResumeSection({ content }: { content: SiteContent }) {
  return (
    <section id="resume" className={homeTokens.sectionSpacing}>
      <SectionHeader title={content.resume.title} intro={content.resume.intro} locale={content.locale} />
      <div className="mt-12 rounded-[32px] bg-panel p-6 shadow-card md:p-8 lg:p-10">
        ...
        <article className="rounded-[24px] bg-white p-6 shadow-card">
          <h3 className={`text-[2rem] leading-tight text-ink ${getDisplayFontClass(content.locale)}`}>
        ...
        <article className="rounded-[24px] bg-white p-6 shadow-card">
          <div>
            <h3 className={`text-[2rem] leading-tight text-ink ${getDisplayFontClass(content.locale)}`}>
              {content.ui.resumeSectionLabels.education}
            </h3>
            <div className="mt-5 space-y-5">
              {content.resume.snapshot.education.map((item) => (
                <div key={`${item.title}-${item.period}`} className="border-l border-slate-200 pl-4">
                  <p className="text-base font-semibold text-ink">{item.title}</p>
                  <p className="mt-1 text-sm text-slate-500">{item.subtitle}</p>
                  <p className="mt-2 font-label text-[11px] uppercase tracking-[0.22em] text-slate-400">{item.period}</p>
          ...
          <div className="mt-8 border-t border-slate-200/80 pt-8">
            <p className={homeTokens.tag}>{content.ui.resumeSectionLabels.awards}</p>
            <div className="mt-4 space-y-5">
              {content.resume.snapshot.awards.map((item) => (
              <div key={`${item.title}-${item.period}`} className="border-l border-slate-200 pl-4">
                <p className="text-base font-semibold text-ink">{item.title}</p>
                <p className="mt-1 text-sm text-slate-500">{item.subtitle}</p>
                <p className="mt-2 font-label text-[11px] uppercase tracking-[0.22em] text-slate-400">{item.period}</p>
        ...
        <article className="rounded-[24px] bg-white p-6 shadow-card">
          ...
          <h3 className={`mt-4 text-[2rem] leading-tight text-ink ${getDisplayFontClass(content.locale)}`}>
